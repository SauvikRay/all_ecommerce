...
class AuthController extends GetxController implements GetxService{
  ...
  Future<ResponseModel> sendOtp(String phone) async {
    ...
    if(response.statusCode == 200){
      ...
      responseModel = ResponseModel(false, 'Something going wrong!',response.body["status"]);
    }
    _isLoading = false;
    update();
  ...
  Future<ResponseModel> verifyOtp(String phone,String verifyOtp) async {
    ...
    if(response.statusCode == 200){
      ...
      if (kDebugMode) {
        print("Otp Verify Response : ${data.toJson().toString()}");
      }
  ...
  Future<ResponseModel> registration(SignUpBody body) async {
    ...
    if(response.statusCode == 200){
      ...
      if (kDebugMode) {
        print("Registration Response : ${registrationResponse.toJson().toString()}");
      }
      ...
      showCustomSnakebar(response.body["message"],title:  response.body["message"],color: Colors.green);
      responseModel = ResponseModel(true, response.body["message"],response.body["status"]);
    }
      ...
      showCustomSnakebar(concatenate.toString(),title:  response.body["message"]);
      //responseModel = ResponseModel(false, response.body["message"],response.body["status"]);
    }
    else{
      responseModel = ResponseModel(false, response.body["message"],response.body["status"]);
  ...
  Future<ResponseModel> login(String email,String password) async {
    ...
    if(response.statusCode == 200){
      ...
      authRepo.saveUserToken(token ?? '');
      saveUserInfo(user.avatarType!,user.name!,user.firstName ?? '',user.lastName ?? '',user.email!,user.createdAt!,user.updatedAt!);
      showCustomSnakebar(response.body["message"],title: response.body["status"],color: Colors.green);
      responseModel = ResponseModel(true, response.body["message"],response.body["status"]);
    }
    else if(response.statusCode == 422){
      errorResponse = RegistrationErrorResponse.fromJson(response.body);
  ...
  void saveUserInfo(String image,String name,String fName,String lName,String email,String createdAt,String lastUpdated){
    authRepo.saveUserInfo(image, name, fName, lName, email, createdAt, lastUpdated);
  }
