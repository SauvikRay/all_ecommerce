...
class ProductController extends GetxController implements GetxService{
  ...
  Future<void> getProductDetails(String slug) async{
    ...
    if(response.statusCode == 200){
      ...
      List<PictureModel>? images = _productDetailModel.productDetails?.pictures!;
      _images = [];
      _images.addAll(images!);
      for(int i=0; i < _images.length; i++){
        _smallImageList.add(SmallImage(i+1, _images[i].url, i+1==1 ? true : false));
      }
      ...
      if(qtyRangeList != null && qtyRangeList.isNotEmpty){
        ...
        List<QuantityRange>? qtyRanges = _productDetailModel.productDetails!.quantityRanges!;
        _quantityRangeList = [];
      ...
      for(int i=0; i < _attributeList.length; i++){
        if(_attributeList[i].propertyName! == "Color" || _attributeList[i].propertyName! == "Machining type") {
          _colorImageList.add(
              ColorImage(i + 1, 0,_attributeList[i].value!, _attributeList[i].imageUrl ?? "", _attributeList[i].vid!,i + 1 == 1 ? true : false)
          );
      ...
      if(_colorImageList != null && _colorImageList.length > 0){
        getSizeListForSpecificColor(_colorImageList != null ? _colorImageList[0].vid : "");
      }
        ...
        if(_configuredItems[i].configurators!.length >= 2){
          ...
          for(int j=0; j <  _configuredItems[i].configurators!.length; j++){
            if(_configuredItems[i].configurators![j].vid! == vid){
              _productSizeList.add(ProductSize(j+1,_configuredItems[i].id! ,_configuredItems[i].configurators![1].vid!,"", _configuredItems[i].quantity!,0, vid, false));
            }
          }
        }else{
          _isSizeExist = false;
